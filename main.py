...
if __name__ == "__main__":
    ...
    for difficulty, num in dict(configParser["difficulty"].items()).items():
        ...
        while num > 0:
            ...
            if (
                not question["isPaidOnly"]
                and doneDict.get(question["title"]) is None
                and likes > dislikes
            ):
                num -= 1
                problems.append(
    ...
    r = requests.patch(
        f"https://api.hackmd.io/v1/notes/{noteID}", headers=headers, json=data
    )
